...
if (process.argv.length < 3) {
  console.error(
    'Argument Error\nUsage:\nnpm start [json file] [OPTIONAL: output directory] [OPTIONAL: file name]\nOR\nnode src/index.js [json file] [OPTIONAL: output directory]'
  );
  return;
}
...
async function run() {
  ...
  const output = path.join(process.argv[3] || path.dirname(process.argv[2]), process.argv[4] || `${song.title}.xml`);
